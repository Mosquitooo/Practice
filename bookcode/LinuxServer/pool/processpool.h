...
private:
	...
	void run_child();
private:
	static const int max_process_number = 16;		//进程池中允许的最大子进程数
	static const int user_per_process = 65535;		//每个子进程最多能处理的客户数
	static const int max_event_number = 10000;		//epoll最多能处理的事件数
	int m_process_number;							//进程池中的进程总数
	int m_idx;										//子进程在进程池中的序号
	int m_epollfd;									//每个进程有一个epoll内核事件表. 用于标识m_epollfd
	int m_listenfd;									//监听socket
	int m_stop;										//子进程通过m_stop来确定是否停止运行
...
static int sig_pipefd[2];   //用于处理信号管道. 以实现统一事件源
	...
	while(!m_stop)
	{
		...
		for(int i = 0; i< number; ++i)
		{
			...
			if(sockfd == m_listenfd)
			{
				...
				printf("send request tcp child %d\n", i);
			}
			else if((sockfd == sig_pipefd[0]) && (events[i].events & EPOLLIN))
			{
				int sig;
